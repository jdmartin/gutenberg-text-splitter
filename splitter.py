...
class Editor:
    ...
    def main(self):
        ...
        def generate_input_list():
            ...
            table = Table(
                title=f"Source Files in [bold cyan]{directory}/[/bold cyan]",
                show_lines=True,
            )
            ...
            def get_file_choice(files):
                ...
                choice = console.input(
                    "Enter the number of the file you'd like to use, '[bold green]D[/bold green]' to delete a file, or '[bold green]M[/bold green]' for the Main Menu: "
                )
                ...
                def remove_the_file(delete_choice, valid_keys):
                    if int(delete_choice) in valid_keys:
                        delete_confirm = Prompt.ask(
                            f"Ok, I'll delete [red]{files.get(int(delete_choice))}[/red]. Is that right? (y/n) "
                        )
        ...
        def get_custom_chapter_prefix():
            if the_program.custom_file_prefix == "":
                choice = console.input(
                    f"Enter a prefix for your chapter files (Format will be '(prefix)chapter_n': "
                )
            else:
                choice = console.input(
                    f"Enter a prefix for your chapter files (Format will be '(prefix)chapter_n', or press enter to keep the current value of [orange1]{the_program.custom_file_prefix}[/orange1]: "
                )
            if choice == "":
                pass
            else:
        ...
        def get_publication_year():
            ...
            if the_program.publication_year == "":
                ...
                choice = console.input(
                    f"Enter the year of publication, or press enter to keep the current value of [blue]{the_program.publication_year}[/blue]: "
                )
            if choice == "":
                pass
            else:
        ...
        def prepare_the_tei_header():
            the_program.title = input(
                "Title of text? (or press enter to skip): "
            ).strip()
            the_program.author = input(
                "Author's name? (or press enter to skip): "
            ).strip()
            the_program.location = input(
                "Location of Publication? (or press enter to skip): "
            ).strip()
            the_program.publisher = input(
                "Publisher? (or press enter to skip): "
            ).strip()
        ...
        def get_element_count_in_chosen_file():
            ...
            suggest_table, suggested_items = (
                offer_suggestions_about_elements_and_attributes(soup)
            )
            ...
            choice = console.input(
                "Give me an element name to look deeper, an option number to use a suggestion, or '[bold green]M[/bold green]' to return to menu: "
            )
            if choice.lower() == "m":
                menu()
        ...
        def offer_suggestions_about_elements_and_attributes(soup):
            ...
            for item in chapter_search_terms:
                ...
                if class_count > 0:
                    option += 1
                    chapter_search_results.append(
                        [element_name, f"{item}", class_count, str(option)]
                    )
            ...
            for item in header_search_terms:
                ...
                if class_count > 0:
                    option += 1
                    header_search_results.append(
                        [element_name, f"{item}", class_count, str(option)]
                    )

            suggest_table = Table(
                title=f"Suggested Elements and Attributes in {the_program.chosen_file}",
                min_width=60,
                show_lines=True,
            )
        ...
        def dig_deeper(selected_element, soup):
            ...
            sort_class_counts_dict = sorted(
                class_counts.items(), key=lambda x: x[1], reverse=True
            )
            sort_id_counts_dict = sorted(
            ...
            print(
                "[bold green]3[/bold green]\tNegative search (experimental, div attributes only for now)"
            )
            print("[bold green]M[/bold green]\tBack to main menu")
            print("\n")

            choice = console.input(
                "Select an attribute to use for chapter selection, or another menu choice: "
            )
            ...
            if choice in class_counts.keys() or choice in id_counts.keys():
                ...
                the_program.selected_element_for_chapters = selected_element
                negative_choices = console.input(
                    "Enter a comma-separated list of attributes to exclude: "
                )
                if negative_choices != "":
                    for neg_choice in negative_choices.split(","):
                        the_program.excluded_attribs_for_chapters.append(
                            neg_choice.strip()
                        )
            elif choice.lower() == "m":
                pass
        ...
        def see_samples_chooser(element):
            ...
            if element in container_elems:
                see_samples_of_container_element(
                    the_program.chosen_file,
                    the_program.selected_element_for_chapters,
                    the_program.selected_attrib_for_chapters,
                )
            else:
                see_samples_of_non_container_element(
                    the_program.chosen_file,
                    the_program.selected_element_for_chapters,
                    the_program.selected_attrib_for_chapters,
                )
        ...
        def show_the_samples(temp_samples):
            ...
            j = 1
            table = Table(
                title="Starting Position and Samples", show_lines=True, expand=True
            )
        ...
        def generate_menu_meta_table():
            ...
            table.add_column(
                "Current File", justify="center", style="cyan", no_wrap=True
            )
            ...
            if (
                the_program.title != ""
                or the_program.author != ""
                or the_program.publisher != ""
                or the_program.location != ""
            ):
                    ...
                    for sibling in elem.next_siblings:
                        if "PROJECT GUTENBERG EBOOK" in sibling.text:
                            if i >= start_pos:
                                if type_of_file == "tei":
                                    with open(
                                        f"output/{output_dir_part}/{the_program.custom_file_prefix}tei_chapter_"
                                        + str(chapter_count),
                                        "w",
                                        encoding="utf-8",
                                    ) as output_file:
                                    ...
                                    with open(
                                        f"output/{output_dir_part}/{the_program.custom_file_prefix}chapter_"
                                        + str(chapter_count),
                                        "w",
                                        encoding="utf-8",
                                    ) as output_file:
                        ...
                        if (
                            sibling.next_element.name == f"{element}"
                            or sibling.next_sibling == None
                        ):
                            if i >= start_pos:
                                if type_of_file == "tei":
                                    with open(
        ...
        def menu():
            ...
            print("\n")
            print(
                "[bold green]F[/bold green]\tFind and Download a File from Proj. Gutenberg"
            )
